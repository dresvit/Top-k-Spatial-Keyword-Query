...
bool viable(int doc, int dep) {
	double B = W[doc];
	if (E[doc] < (1<<(m+1))-1) {
		for (int i = 1; i <= m; i++)
			if (((1<<i) & E[doc]) == 0)
				B += alpha*scoreList[pc[i]][dep].score;
		if ((1 & E[doc]) == 0)
...
int main() {
	freopen("answer_CA.txt", "w", stdout);
