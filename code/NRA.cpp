...
int WChangeNum, WChangeList[maxb];
int posHeap[maxn];
double alpha, gamma, Wk, Bk;
double dis[maxn], bottom[maxk];
...
void seqAccess(int doc, int x, double score) {
	if (((1<<x) & E[doc]) == 0) {
		...
		bool flag = false;
		for (int i = 1; i <= WChangeNum; i++)
			if (WChangeList[i] == doc) {
				flag = true;
				break;
			}
		if (!flag) WChangeList[++WChangeNum] = doc;
		W[doc] += score;
		E[doc] |= 1<<x;
		if (E[doc] == (1<<(m+1))-1) B[doc] = W[doc];
	}
}
...
void updateB() {
	for (int i = 1; i <= recNum; i++) {
		...
		if (E[doc] < (1<<(m+1))-1) {
			...
			for (int j = 1; j <= m; j++)
				if (((1<<j) & E[doc]) == 0)
					B[doc] += alpha*bottom[j];
			if ((1 & E[doc]) == 0)
				B[doc] += (1-alpha)*bottom[0];
		}
	}
}

int findRandomAccessObject() {
	int doc = -1;
	for (int i = 1; i <= recNum; i++) {
		int now = recList[i];
		if (E[now] < (1<<(m+1))-1 && B[now] > Wk && (doc == -1 || B[doc] < B[now]))
			doc = now;
	}
	return doc;
}
...
bool canHalt() {
	...
	while (refind) {
		...
		tmpNum = 0;
		for (int i = 1; i <= K; i++) {
			int doc = topkHeap.heap[i].id;
			if (E[doc] < (1<<(m+1))-1) tmpList[++tmpNum] = doc;
		}
		for (int i = 1; i <= tmpNum; i++)
			randomAccess(tmpList[i]);
		if (refind) {
			for (int i = 1; i <= recNum; i++)
				updateTopk(recList[i]);
		}
		if (Bk > Wk) return false;
	}
	return true;
}
...
void solve() {
	...
	for (int i = 1; i <= n; i++) {
		...
		for (int j = 0; j <= m; j++)
			if (i <= Listlength[pc[j]]) {
				bottom[j] = scoreList[pc[j]][i].score;
				Bk += bottom[j];
			}
			else
				bottom[j] = 0;
		
		for (int j = 1; j <= WChangeNum; j++)
			updateTopk(WChangeList[j]);
		WChangeNum = 0;
...
int main() {
	freopen("answer_NRA.txt", "w", stdout);
...
//	printf("Read data time: %.6lfs\n", elapsedTime_read);
	printf("Solve time: %.6lfs\n", elapsedTime_solve);
